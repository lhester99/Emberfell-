...
(function () {
  ...
  /* Placement: if the objective gives ringMin/ringMax (multipliers of the POI
   * radius) we ring the pickups OUTSIDE the radius - on reachable ground past
   * a flattened/water POI. Otherwise we scatter inside (legacy). */
  function spawnPoint(poi, sp, k, n) {
    var rad = poi.radius || 6;
    var a = (k / n) * Math.PI * 2 + (Math.random() - 0.5) * (Math.PI / n);
    var d;
    if (sp.ringMin != null || sp.ringMax != null) {
      var rMin = (sp.ringMin != null ? sp.ringMin : 1.1) * rad;
      var rMax = (sp.ringMax != null ? sp.ringMax : 1.3) * rad;
      d = rMin + Math.random() * (rMax - rMin);
    } else {
      var scatter = sp.scatter || 5;
      d = scatter * (0.55 + Math.random() * 0.45);
    }
  ...
  function spawnCollectPickups(q, rec) {
    for (var i = 0; i < q.objectives.length; i++) {
      ...
      // capture q.id/rec so a deferred spawn can verify the quest is still the
      // same active record (not abandoned/re-accepted) before spawning.
      (function (objSpawn, count, questId, forRec) {
        whenSpawnable(function () {
          if (records[questId] !== forRec || forRec.status === 'done') return;
          for (var k = 0; k < count; k++) {
            var pt = spawnPoint(poi, objSpawn, k, count);
            var h = EF.world.spawnPickup(o.item, pt.x, pt.z);
